...
class RegisterDetails extends React.Component {
	...
	validateForm(e) {
		...
		let platforms = document.getElementById("select-multiple-checkbox-platform").value;
		let lobmultiple = document.getElementById("select-multiple-checkbox").value;
		if (!company_name && !company_entity && !company_website && !company_phone_countrycode && !company_phone && !street_address && !hstate && !hcountry && !hcity && !hzip
			&& !platforms && !lobmultiple && !vnf_category && !tags && !product_name && !product_typestate && !tags && !description
			&& !pccountry && !pcfn && !pcln && !pcemail && !pcphone && !pccountrycode && !vzwcontactText) {
